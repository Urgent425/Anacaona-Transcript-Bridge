// src/pages/AdminOfficialFiles.jsx
...
export default function AdminOfficialFiles() {
  ...
  const rowKey = (r) => `${r.transcriptId}-v${r.upload?.version || 1}`;

  const setUploadStatus = async (r, newStatus) => {
    const ver = r.upload?.version || 1;
    const key = rowKey(r);
    ...
    try {
      ...
      const res = await fetch(
        `${API_BASE}/api/admin/officials/${r.transcriptId}/${ver}/status`,
        {
          method: "PATCH",
          headers: {
      ...
      if (!res.ok) {
        ...
        throw new Error(`Update failed (${res.status}). ${msg}`.trim());
      }
      ...
      setRows((prev) =>
        prev.map((x) => {
          ...
          return { ...x, upload: { ...x.upload, status: newStatus } };
        })
      );
    } catch (e) {
  ...
  const downloadOfficial = async (r) => {
    const ver = r.upload?.version || 1;
    const key = rowKey(r);
    ...
    try {
      ...
      const res = await fetch(
        `${API_BASE}/api/admin/officials/${r.transcriptId}/${ver}/download`,
        { headers: { Authorization: `Bearer ${token}` } }
      );
  ...
  return (
    <div className="p-4">
      ...
      <div className="flex gap-2 mb-3">
        ...
        >
          {["clean", "pending_scan", "infected", "ALL"].map((s) => (
        ...
        <input
          className="border rounded px-2 py-1"
          placeholder="Search submissionId/name/email"
          value={q}
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {rows.length ? (
              rows.map((r, i) => {
                const key = rowKey(r);
                const currentStatus = r.upload?.status || "pending_scan";
                const isClean = currentStatus === "clean";

                return (
                  <tr key={`${key}-${i}`} className="border-t">
                    <td className="px-3 py-2 font-mono">
                      {r.submissionId || r.transcriptId?.slice(-8)}
                    </td>
                    ...
                    <td className="px-3 py-2">
                      {r.upload?.uploadedAt ? new Date(r.upload.uploadedAt).toLocaleString() : "—"}
                    </td>
                    ...
                    <td className="px-3 py-2">
                      <select
                        ...
                        onChange={(e) => setUploadStatus(r, e.target.value)}
                      >
                        {["pending_scan", "clean", "infected"].map((s) => (
                          <option key={s} value={s}>
                            {s}
                          </option>
                        ))}
